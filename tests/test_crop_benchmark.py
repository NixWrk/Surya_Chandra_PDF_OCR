...
def test_run_crop_benchmark_preserves_natural_input_order(tmp_path, monkeypatch) -> None:
    ...
    assert [(backend, mean) for backend, mean in seen_means] == [
        ...
        (DETECTOR_BACKEND_UVDOC, 30),
        (DETECTOR_BACKEND_UVDOC, 90),
    ]
    assert [result.backend for result in results] == [DETECTOR_BACKEND_OPENCV, DETECTOR_BACKEND_UVDOC]
    ...
    assert exported == {
        ...
        "input_uvdoc": ["00001.png", "00002.png"],
    }
...
def test_run_crop_benchmark_keeps_other_backends_when_one_is_unavailable(tmp_path, monkeypatch) -> None:
    ...
    assert failed_result.backend == DETECTOR_BACKEND_UVDOC
        ...
        return [
            type(
                ...
                (),
                {
                    ...
                    "output_pdf": output_dir / "input_opencv_quad.pdf",
                    "detected_pages": 2,
                    "total_pages": 2,
                    "error": None,
                },
